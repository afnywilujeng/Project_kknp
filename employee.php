...
include("config.php");
 ...
 try {
    ...
    if (!$ids)
       throw new Exception('profil employee tidak ditemukan');
    ...
    $key = array(
       ...
       new xmlrpcval('work_location', 'string'),
    );
    $msg = new xmlrpcmsg('execute');
    $msg->addParam(new xmlrpcval($dbname, "string"));
    $msg->addParam(new xmlrpcval($user_id, "int"));
 ...
 <section id="main" class="column"><!-- end of stats article --><!-- end of content manager article -->
  ...
  <div class="clear"></div>
    ...
    <article class="module width_full">
       <header><h3>List Employee</h3></header>
             ...
             <?php if ($myCalenders) { ?>
                <table class="tablesorter">
                   ...
                   <?php foreach ($myCalenders as $mc) { ?>
                      <?php $m = $mc->scalarval(); ?>
                      <tr>
                      <td><a href="profil_admin.php?id=<?php echo $m['id']->scalarval()?>">
                        <?php echo $m['name']->scalarval() ?></a></td>
